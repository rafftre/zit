...
pub fn repository(self: *GitRepository, objects: ObjectStore) Repository {
    return .{
        .ptr = self,
        .objects = objects,
        .openFn = open,
...
pub fn init(allocator: Allocator, dir_name: ?[]const u8) !GitRepository {
    ...
    return .{ .git_dir_path = git_dir_path };
}
