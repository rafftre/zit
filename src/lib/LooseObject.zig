...
//! An object in raw binary format.
//!
//! It is a sequence of bytes formed by
//! - an header (object type name + space + data length),
//! - a null byte,
//! - the serialized binary data of the object.
//!
//! An instance of this struct can be the result of a fully determined object (to be written to the object store)
//! or a set of parsed data (read from the object store) that may be incomplete or erroneous.
//!
//! When parsed, it retains the raw type name and length as they are read.

const LooseObject = @This();
...
/// Initializes a loose object for a known type.
pub fn init(obj_type: ObjectType, data: []u8) LooseObject {
    return .{
        .data = data,
...
/// Encodes the loose object as a sequence of bytes.
pub fn encode(self: *const LooseObject, allocator: Allocator) ![]u8 {
...
/// Decodes the sequence of bytes into a loose object.
/// `hasher` is used to verify the content if the options have `expected_id`.
pub fn decode(bytes: []u8, hasher: anytype, options: DecodeOptions) !LooseObject {
    ...
    if (options.expected_id) |check_id| {
        ...
        hasher.hashData(bytes, &obj_id.bytes);

        if (!check_id.eql(&obj_id)) {
            std.log.debug("Found mismatching object ID: '{any}'", .{obj_id});
...
test "decode" {
    const hasher = @import("helpers.zig").hash.Sha1;
    ...
    const test_id = try ObjectId.parseHex(test_hex);

    const decoded = try decode(@constCast(test_bytes), hasher, .{
...
test "decode errors" {
    ...
    const test_id = try ObjectId.parseHex("30f51a3fba5274d53522d0f19748456974647b4f");

    // read known type while allowing unknown
    const known = try decode(@constCast("blob 13\x00hello, world!"), hasher, .{ .allow_unknown_type = true });
    ...
    // read unknown type
    const unk = try decode(@constCast("test 13\x00hello, world!"), hasher, .{ .allow_unknown_type = true });
    ...
    // read empty data
    const nodata = try decode(@constCast("blob 0\x00"), hasher, .{});
    ...
    try std.testing.expectError(
        error.UnknownType,
        decode(@constCast("test 13\x00hello, world!"), hasher, .{ .allow_unknown_type = false }),
    );

    // mismatching type error
    try std.testing.expectError(
        error.TypeMismatch,
        decode(@constCast("tree 13\x00hello, world!"), hasher, .{ .expected_type = .blob }),
    );

    // mismatching length error
    try std.testing.expectError(error.LengthMismatch, decode(@constCast("blob 12\x00hello, world!"), hasher, .{}));

    // bad length error
    try std.testing.expectError(error.BadLength, decode(@constCast("blob aa\x00hello, world!"), hasher, .{}));

    // mismatching object ID error
    try std.testing.expectError(
        error.ObjectIdMismatch,
        decode(@constCast("blob 13\x00hello, world"), hasher, .{ .expected_id = test_id }),
    );

    // missing header error
    try std.testing.expectError(error.MissingHeader, decode(@constCast("blob 130hello, world!"), hasher, .{}));
    try std.testing.expectError(error.MissingHeader, decode(@constCast("hello, world!"), hasher, .{}));
    try std.testing.expectError(error.MissingHeader, decode(@constCast("hello,world!"), hasher, .{}));
    try std.testing.expectError(error.MissingHeader, decode(@constCast(""), hasher, .{}));

    // malformed header error
    try std.testing.expectError(error.MalformedHeader, decode(@constCast("blob13\x00hello, world!"), hasher, .{}));
    try std.testing.expectError(error.MalformedHeader, decode(@constCast("blob13\x00hello,world!"), hasher, .{}));
}
