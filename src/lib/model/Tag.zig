...
pub inline fn interface(self: Tag) Object {
    return .{ .tag = self };
}
...
/// Free with `deinit`.
pub fn deserialize(allocator: Allocator, data: []const u8) !Tag {
    ...
    errdefer {
        ...
        if (tagger) |t| t.deinit(allocator);
    }
        ...
        if (in_message) {
            ...
            if (space_idx) |i| {
                ...
                if (std.mem.eql(u8, header, "object") == true) {
                    object_id = try ObjectId.parseHex(value);
                    ...
                    tagger = try Signature.deserialize(allocator, value);
                }
            }
        }
    }
...
test "serialize and deserialize" {
    ...
    const oid = try ObjectId.parseHex("1234567890abcdef1234567890abcdef12345678");
