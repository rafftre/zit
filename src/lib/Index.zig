...
pub const IndexExtension = @import("index_extension.zig").IndexExtension;
...
inline fn deinitEntries(entries: std.ArrayList(IndexEntry)) void {
...
/// Free with `deinit`.
pub fn parse(allocator: Allocator, data: []u8) !Index {
    ...
    // entries
    var entries = try std.ArrayList(IndexEntry).initCapacity(allocator, entry_count);
    errdefer deinitEntries(entries);

    for (0..entry_count) |_| {
        const parsed = try IndexEntry.parse(allocator, data[pos..], version);
    ...
    var checksum = ObjectId.init(.{});
    @memcpy(&checksum.hash, data[pos..(pos + hash_size)]);

    // verify checksum
    var calculated_checksum = ObjectId.init(.{});
    hasher.final(&calculated_checksum);

    if (!checksum.eql(&calculated_checksum)) {
...
pub fn writeTo(self: *const Index, buffer: *std.ArrayList(u8)) !void {
    // header
    ...
    // checksum
    const checksum = Hasher.hashData(buffer.items);
    try buffer.appendSlice(&checksum.hash);
}
...
test "index" {
    ...
    var expected: Index = .{
        ...
        .checksum = try ObjectId.parseHexString("6352c0839c74a97089c08761e42b180d62a9dad6"),
    };
    ...
    try expected.entries.append(.{
        ...
        .group_id = 1000,
        .file_size = 2,
        .object_id = try ObjectId.parseHexString("0123456789abcdeffedcba98765432100f1e2d3c"),
        .flags = IndexEntry.Flags{ .assume_valid = true, .name_length = 8 },
    ...
    try std.testing.expect(parsed.checksum.eql(&expected.checksum));
...
test "index errors" {
    ...
    try std.testing.expectError(error.UnexpectedEndOfFile, parse(allocator, too_short1));
    ...
    try std.testing.expectError(error.UnexpectedEndOfFile, parse(allocator, too_short2));
    ...
    try std.testing.expectError(error.InvalidSignature, parse(allocator, invalid_sign));
    ...
    try std.testing.expectError(error.UnsupportedVersion, parse(allocator, unsupported_version));
    ...
    try std.testing.expectError(error.InvalidFormat, parse(allocator, invalid_format));
    ...
    try std.testing.expectError(error.InvalidChecksum, parse(allocator, invalid_checksum));
}
