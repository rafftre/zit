...
const FileMode = @import("core/file_mode.zig").FileMode;
...
pub fn deinit(self: *const IndexEntry, allocator: Allocator) void {
...
pub fn parse(allocator: Allocator, data: []u8, version: u32) !struct {
    ...
    const entry: IndexEntry = .{
        ...
        .object_id = ObjectId.init(.{ .hash = data[40..60].* }),
        .flags = flags,
...
pub fn writeTo(self: *IndexEntry, buffer: *std.ArrayList(u8), version: u32) !void {
    ...
    try buffer.appendSlice(&self.object_id.hash);
...
test "entry" {
    ...
    const test_cases = [_]struct { u32, []u8, IndexEntry }{
        .{
            2,
            @constCast(&[_]u8{
...
fn expectEntry(allocator: Allocator, bytes: []u8, expected: *IndexEntry, version: u32) !void {
    const parsed = try parse(allocator, bytes, version);
...
test "entry errors" {
    ...
    try std.testing.expectError(error.UnexpectedEndOfFile, parse(allocator, too_short_v2, 2));
    ...
    try std.testing.expectError(error.UnexpectedEndOfFile, parse(allocator, too_short_v3, 3));
    ...
    try std.testing.expectError(error.UnexpectedEndOfFile, parse(allocator, too_short_path_name, 2));
}
